...
export const REPORTES = {
  ...
  // ── FLOTA — idéntico a hoja "Flota" del Excel control-de-flota-vehicular.xlsx
  // Columnas y orden exactos de la plantilla: 16 columnas, grupo "DATOS PRINCIPALES" (C-L) y "MANTENIMIENTO" (M-R)
  camiones: {
    ...
    columnas: [
      ...
      ['Kms Odómetro Mnnto. Preventivo', 16], ['Kms Odómetro Actual', 14],
      ['Falta para el próximo Mnnto Prev. (Km)', 13], ['Estado Mantenimiento Preventivo', 16],
    ],
    sql: `
    ...
    fila: r => {
      ...
      // Los campos de odómetro preventivo/actual dependen de módulos Rutas/Mantenimiento aún no implementados
      // Se dejan vacíos igual que en la plantilla cuando no hay datos (fórmulas MAXIFS devuelven vacío)
      return [
  ...
  // ── MULTAS — idéntico a hoja "Multas" (11 columnas) con fórmulas Debe y Estado Pago
  multas: {
    ...
    sql: `
      SELECT m.id, m.fecha, m.motivo AS infraccion, m.monto AS importe_multa, m.observaciones,
             ch.nombre AS conductor, ch.placa AS placa_vehiculo
      FROM multas m JOIN choferes ch ON ch.id = m.chofer_id
      WHERE ${EN_MES('m.fecha')}
      ORDER BY m.fecha ASC`,
    fila: r => {
      const importe = r.importe_multa !== null ? Number(r.importe_multa) : 0;
      const pagado = 0; // No existe columna importe_pagado en DB; se registra como 0 (igual que plantilla vacía)
      const debe = importe - pagado;
      const estadoPago = debe === 0 ? 'Pagado' : 'Pendiente';
      return [
        r.id, fmtDia(r.fecha), '', r.placa_vehiculo || '', r.conductor || '', r.infraccion || '',
